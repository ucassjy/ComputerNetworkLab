		...
		struct stp_config *config)
{
	...
	else if (p->designated_root < ntohll(config->root_id))       pri = 0;
	else if (p->designated_cost > ntohl(config->root_path_cost)) pri = 1;
	else if (p->designated_cost < ntohl(config->root_path_cost)) pri = 0;
	else if (p->designated_switch > ntohll(config->switch_id))   pri = 1;
	else if (p->designated_switch < ntohll(config->switch_id))   pri = 0;
	else if (p->designated_port > ntohs(config->port_id))        pri = 1;
	if (pri == 1) {
		// Config received has higher priority.
		// Update Config for this port.
		...
		for (int i = root + 1; i < stp->nports; i++) {
			...
			else if (stp->ports[i].designated_root < stp->ports[root].designated_root)     pri = 1;
			else if (stp->ports[i].designated_cost > stp->ports[root].designated_cost)     pri = 0;
			else if (stp->ports[i].designated_cost < stp->ports[root].designated_cost)     pri = 1;
