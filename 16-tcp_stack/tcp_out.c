...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
	...
	struct snd_buf sndb;
	sndb.seq_end = tsk->snd_nxt - 1;
	sndb.packet = (char*)malloc(tcp_data_len * sizeof(char));
	memcpy(sndb.packet, packet + len - tcp_data_len, tcp_data_len);
	list_add_tail(&sndb.list, &tsk->snd_buffer);
