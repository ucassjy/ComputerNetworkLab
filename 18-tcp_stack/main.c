...
static void usage_and_exit(const char *basename)
{
	...
	fprintf(stderr, "\t%s worker local_port\n", basename);
	fprintf(stderr, "\t%s master remote_port\n", basename);
...
static void run_application(const char *basename, char **args, int n)
{
	...
	if (strcmp(args[0], "master") == 0) {
		...
		pthread_create(&thread, NULL, tcp_master, &port);
	}
	else if (strcmp(args[0], "worker") == 0) {
		if (n != 2)
			usage_and_exit(basename);

		u16 port = htons(atoi(args[1]));
		pthread_create(&thread, NULL, tcp_worker, &port);
	}
	else {
		usage_and_exit(basename);
	}
